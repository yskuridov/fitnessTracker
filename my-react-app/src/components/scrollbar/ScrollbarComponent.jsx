...
function ScrollbarComponent({ username, date }) {
    ...
    return (
        dailyExercises.length > 0 || dailyMeals.length > 0 ? (
            <div className='container' style={{ position: 'relative', overflow: 'hidden' }}>
                <div className='row'>
                    <div className='col-5'>
                        ...
                        <div style={{ display: 'flex', overflowX: 'auto', width: '100%', scrollbarWidth: 'none' }} ref={mealContainerRef}>
                            {dailyMeals.map((meal, index) => (
                                <DailyMealComponent key={index} id={meal.mealDto.id} name={meal.mealDto.name} ingredients={meal.mealDto.ingredients} nutrients={{ 'calories': meal.mealDto.calories, 'protein': meal.mealDto.protein, 'carbs': meal.mealDto.carbs, 'fat': meal.mealDto.fat, 'fiber': meal.mealDto.fiber, 'calcium': meal.mealDto.calcium, 'sodium': meal.mealDto.sodium, 'cholesterol': meal.mealDto.cholesterol }} image={meal.mealDto.image} servingWeight={meal.mealDto.servingPortion} instructions={meal.mealDto.instructions} summaryDate={date} handleDelete={handleDelete}/>
                            ))}
                        </div>
                        <div className='mx-auto my-auto'>
                            <button
                    ...
                    <div className='col-7'>
                        <div className='row border border-info border-2 border-top-0 border-start-1 border-bottom-0 border-end-0' >
                            <h3 className='text-center text-info mt-3'>Analyse</h3>
                            <div className='col-6'>
                            <NutritionTableComponent exercises={dailyExercises} meals={dailyMeals}/>
                            <p className='fs-6 text-info mt-3 mb-0'>Vos données</p>
                            </div>
                            <div className='col-6'>
                            <NutritionTableComponent exercises={dailyExercises} meals={dailyMeals}/>
                            <p className='fs-6 text-success mt-3 mb-0'>Objectif à viser</p>
                            </div>
        ...
        ) : (<div>Vous n'avez pas encore planifié votre journée</div>)
    );
}
